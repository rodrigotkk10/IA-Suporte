<?php
// database.php - Classe para gerenciamento do banco de dados
...
class Database {
    ...
    public function cadastrarConvenio($dados) {
        $sql = "INSERT INTO convenios (nome, margem, segmento, ativo, created_at) VALUES (?, ?, ?, 1, NOW())";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$dados['nome'], $dados['margem'], $dados['segmento']]);
    }
    
    public function atualizarConvenio($id, $dados) {
        $sql = "UPDATE convenios SET nome = ?, margem = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function cadastrarProduto($dados) {
        $sql = "INSERT INTO produtos (nome, prazo, taxa, convenio_id, ativo, created_at) VALUES (?, ?, ?, ?, 1, NOW())";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function criarTabelas() {
        $sqls = [
            "CREATE TABLE IF NOT EXISTS convenios (
                ...
                segmento ENUM('consignado', 'protecao_veicular', 'construtora', 'condominio') DEFAULT 'consignado',
